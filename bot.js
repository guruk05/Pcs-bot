...
console.log(config.CHAIN, config.NETWORK);

const data = {
  BNB: config.BNB_CONTRACT, //bnb

  to_PURCHASE: config.TO_PURCHASE, // token that you will purchase = BUSD for test '0xe9e7cea3dedca5984780bafc599bd69add087d56'

  AMOUNT_OF_BNB: config.AMOUNT_OF_BNB, // how much you want to buy in BNB

  factory: config.FACTORY, //PancakeSwap V2 factory

  router: config.ROUTER, //PancakeSwap V2 router
...
const wss = config.WSS_NODE;
const mnemonic = config.YOUR_MNEMONIC; //your memonic;
...
const approve = async () => {
  try {
    ...
    console.log("Approved " + tokenOut);
    console.log("Your txHash: " + receipt.transactionHash);
    console.log(
      `Transaction receipt : ${data.explorerUrl}/tx/${receipt.transactionHash}`
    );
  } catch (err) {
